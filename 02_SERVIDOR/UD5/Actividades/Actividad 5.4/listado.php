...
if (isset($_POST['comprar'])) {
    ...
    if ($datos !== false) {
        ...
        if (!isset($_COOKIE['familias'])) {
            setcookie("familias[0]", $familia, 3600 * 24 * 30 + time());
            $familiaArray = $_COOKIE['familias'];
            $cantidad = count($_COOKIE['familias']);

            if (!in_array($familia, $familiaArray)) {
                setcookie("familias[$cantidad]", $familia, 3600 * 24 * 30 + time());
            }
            var_dump( $familiaArray) ;
        }
    }
}
